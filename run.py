...
observations = pro.str2words(test)
...
B = pro.BIOconf(X, word_count, test_wordcount, hidden_states)
phi = {'B': 0.9 ,'I': 0,'O': 0.1}
A = [[0.45, 0.35, 0.2], [0.6, 0.2, 0.2], [1, 0, 0]] 
model = Model(S, observation, phi, A, B)
phi, A, B = model.learningPhase(pro.str2words(X), 3, A, B)
o_hiddenstate = []
...
word_sequence = pro.word_sequence(observations, o_hiddenstate)
print(word_sequence)
